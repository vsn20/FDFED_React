...
const Companymessages = () => {
  ...
  // Fetch Messages based on view and filters
  useEffect(() => {
  ...
  // Fetch Sales Managers only when entering Compose view
  useEffect(() => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setView('sent'); // Redirect to Sent folder
      } else {
  ...
  const handleBack = () => {
    if (view === 'details') {
      // Return to the previous list view (Inbox/Sent)
      // Since 'selectedMessage' tells us where it's from (implicitly), 
      // but simpler is to check 'fromDisplay' == 'You' to guess it was Sent view?
      // Or just default to Inbox if state was lost, but usually we just want to go back to what we were looking at.
      // However, simplified approach: if it was a sent message, go sent.
      if (selectedMessage.fromDisplay === 'You') {
        setView('sent');
      } else {
        setView('inbox');
      }
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {messages.length > 0 ? (
              ...
              <tr>
                <td colSpan="3" style={{...styles.td, textAlign: 'center'}}>No messages found.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {totalPages > 1 && (
        <div style={styles.pagination}>
          <button 
            ...
            style={{...styles.btn, ...styles.inactiveBtn, cursor: currentPage === 1 ? 'not-allowed' : 'pointer'}}
          >
            Prev
          </button>
          ...
          <button 
            ...
            style={{...styles.btn, ...styles.inactiveBtn, cursor: currentPage === totalPages ? 'not-allowed' : 'pointer'}}
          >
            Next
          </button>
        </div>
      ...
      <div style={styles.container}>
        <div style={styles.contentArea}>
          ...
          <div style={styles.header}>
            ...
            {/* Navigation Buttons (Hide in details/compose view to avoid clutter, or keep them) */}
            {view !== 'details' && view !== 'compose' && (
              <div style={styles.navButtons}>
                <button 
                  onClick={() => { setView('inbox'); setCurrentPage(1); }}
                  style={{...styles.btn, ...(view === 'inbox' ? styles.activeBtn : styles.inactiveBtn)}}
                >
                  Inbox
                </button>
                <button 
                  onClick={() => { setView('sent'); setCurrentPage(1); }}
                  style={{...styles.btn, ...(view === 'sent' ? styles.activeBtn : styles.inactiveBtn)}}
                >
                  Sent
                </button>
                <button 
                  onClick={() => setView('compose')}
                  style={{...styles.btn, backgroundColor: '#0984e3', color: 'white'}}
                >
                  Compose
                </button>
              </div>
          ...
          {view === 'details' && selectedMessage && (
            <div>
              <button onClick={handleBack} style={styles.backBtn}>
                &larr; Back to {selectedMessage.fromDisplay === 'You' ? 'Sent' : 'Inbox'}
              </button>
              <div style={styles.detailCard}>
                <div style={styles.detailRow}>
                  <span style={styles.label}>From:</span>
                  <span style={styles.value}>{selectedMessage.fromDisplay}</span>
                </div>
                <div style={styles.detailRow}>
                  <span style={styles.label}>To:</span>
                  <span style={styles.value}>{selectedMessage.toDisplay}</span>
                </div>
                <div style={styles.detailRow}>
                  <span style={styles.label}>Date:</span>
                  <span style={styles.value}>{new Date(selectedMessage.timestamp).toLocaleString()}</span>
                </div>
                <hr style={{margin: '15px 0', border: '0', borderTop: '1px solid #dfe6e9'}} />
                <div style={styles.detailRow}>
                  <span style={styles.label}>Message:</span>
                  <div style={styles.value}>{selectedMessage.message}</div>
                </div>
              </div>
            </div>
          )}
                  ...
                  <div style={styles.formGroup}>
                    ...
                    >
                      ...
                      {salesManagers.map(mgr => (
                        <option key={mgr.e_id} value={mgr.e_id}>
                          {mgr.e_id} - {mgr.f_name} {mgr.last_name}
                        </option>
                      ))}
                    </select>
                  </div>
